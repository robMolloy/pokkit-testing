...
const testDirPath = `_temp/organisationDocumentCollectionUpdateRules`;

const testDbUrl = `http://0.0.0.0:8054`;
...
describe(`organisation documents collection update rules - happy and unhappy paths`, () => {
  beforeAll(async () => {
  ...
  it(`allows user to update an organisation document record file if;
  ...
  `, async () => {
    ...
    expect(records.map((doc) => doc.fileName)).toEqual(["image2.png", "image2.png"]);
  });

  it(`denies user to update an organisation document record file if;
      - standard orgUserPermission record
  `, async () => {
    const { orgStandardUserPb, organisationRecord, organisationDocumentRecord1 } =
      await setupOrgDocumentRecordsForUpdateTests();

    await expect(
      orgStandardUserPb.collection(organisationDocumentsCollectionName).update(
        organisationDocumentRecord1.id,
        organisationDocumentSeedFactory.forCreate({
          file: await getImage2File(),
          organisationId: organisationRecord.id,
  ...
  it(`denies user to update an organisation document record file if;
      - not in org
  `, async () => {
    const { notInOrgUserPb, organisationRecord, organisationDocumentRecord1 } =
      await setupOrgDocumentRecordsForUpdateTests();

    await expect(
      notInOrgUserPb.collection(organisationDocumentsCollectionName).update(
        organisationDocumentRecord1.id,
        organisationDocumentSeedFactory.forCreate({
          file: await getImage2File(),
          organisationId: organisationRecord.id,
