...
const setupOrgNotesRecordForUpdateTests = async () => {
  ...
  const organisationNoteRecord = await globalAndOrgAdminUserPb
...
describe(`organisation notes collection update rules - happy path`, () => {
  beforeEach(async () => {
    await clearDatabase();
  });

  it(`allows user to update an organisation note record if;
      - admin orgUserPermission
  `, async () => {
    const { globalAndOrgAdminUserPb, orgAdminUserPb, organisationNoteRecord } =
      await setupOrgNotesRecordForUpdateTests();

    const organisationNoteRecordResp1 = await globalAndOrgAdminUserPb
      .collection(organisationNotesCollectionName)
      .update(organisationNoteRecord.id, { markdown: "updated markdown content" });
    expect(organisationNoteRecordResp1).toBeTruthy();

    const organisationNoteRecordResp2 = await orgAdminUserPb
      .collection(organisationNotesCollectionName)
      .update(organisationNoteRecord.id, { markdown: "updated markdown content 2" });
    expect(organisationNoteRecordResp2).toBeTruthy();
  });
});

describe(`organisation notes collection update rules - unhappy paths`, () => {
  beforeEach(async () => await clearDatabase());

  it(`denies user to update an organisation note record if;
      - standard orgUserPermission record
  `, async () => {
    const { orgStandardUserPb, organisationNoteRecord } = await setupOrgNotesRecordForUpdateTests();
